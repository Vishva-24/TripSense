...
import { ArrowLeft, Check, Clock3, MapPin, Pencil, Users, X } from "lucide-react";
...
function getFallbackImage(title, destination) {
  return `https://placehold.co/480x320/png?text=${encodeURIComponent(`${title} - ${destination}`)}`;
}

const travelerOptions = ["Solo", "Couple", "Family", "Friends"];
const budgetOptions = ["Shoestring", "Standard", "Luxury"];
const vibeOptions = ["Chill", "Adventure", "Culture", "Party", "Food-focused"];
...
export default function ActiveItineraryPage() {
  ...
  useEffect(() => {
    ...
    const loadTrip = async (tripId) => {
      try {
        ...
        if (!response.ok) {
          throw new Error(result?.details || result?.error || "Could not load trip.");
        }
    ...
    const resolvedTripId = (tripIdFromQuery || localTripId || "").trim();
    ...
    if (typeof window !== "undefined" && tripIdFromQuery) {
      window.localStorage.setItem("tripsense_last_trip_id", tripIdFromQuery);
    }
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !shouldWarnBeforeLeave) return undefined;
    ...
    return apiTripData.days.map((day) => ({
      ...
      activities: (day.activities || []).map((activity) => ({
        ...
        imageUrl:
          activity.imageUrl ||
          getFallbackImage(
            activity.title || "travel location",
            destination
          )
      }))
    }));
  }, [activeTripId, apiTripData, destination]);
  ...
  const packingList = buildPackingList(apiTripData?.trip?.destination || "");
  ...
  return (
    <main className="mx-auto max-w-[1280px] px-4 py-8 md:px-6 md:py-10">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-extrabold text-app-slate">Active Itinerary</h1>
          ...
          <section className="card-surface max-h-[75vh] space-y-6 overflow-y-auto p-5">
            ...
            {normalizedDays.map((day) => (
              <div key={day.dayNumber} className="space-y-3">
                <div className="sticky top-0 z-10 rounded-xl bg-app-sandSoft px-3 py-2">
                ...
                <div className="space-y-3">
                  {day.activities.map((activity) => (
                    <ActivityCard
                      ...
                      rerollDisabled={!apiTripData?.trip?.id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4 py-6">
          <div className="w-full max-w-2xl rounded-2xl border border-app-border bg-white p-5 shadow-xl md:p-6">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-app-slate">Edit Plan Options</h2>
                  ...
                  {dietaryOptions.map((option) => {
                    ...
                    return (
                      <button
                        key={option}
                        type="button"
                        onClick={() => toggleEditMultiOption("dietaryPrefs", option)}
                        className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${
                          selected
                            ? "border-app-slate bg-app-slate text-white"
                            : "border-app-border bg-white text-app-slate hover:bg-app-sky"
